...
def drawxy(img):
    cv2.line(img, (320,0), (320, 480), (0,255,255), 1) # dọc  y
    cv2.line(img, (0,240), (640, 240), (0,255,255), 1) # ngang x
...
def main():
    ...
    while True:
        ...
        x,y,w,h = 0,0,0,0
        fire = fire_cascade.detectMultiScale(image, 1.08, 3)
        ...
        if( w != 0):    
            x1 = x + w + 20
            y1 = y + h + 20
            image_cut = image[y - 20:y1, x - 20:x1] # v
            cv2.imshow("goc", image_cut)
            
            image_cut = cv2.addWeighted(image_cut, 1, image_cut, 0, -75) #giảm độ sáng
            cv2.imshow("dark", image_cut)
